...
inputs = merge(
  ...
  {
    host                     = dependency.eks_2.outputs.cluster_endpoint
    cluster_ca_certificate   = base64decode(dependency.eks_2.outputs.cluster_certificate_authority_data)
    cluster_id               = dependency.eks_2.outputs.cluster_id
    worker_security_group_id = dependency.eks_2.outputs.worker_security_group_id
  },
  {
